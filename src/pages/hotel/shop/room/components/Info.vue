...
const hotelName = computed(
  () =>
    hotelOptions.value.find((item) => item.id === roomInfo.value.hotelId)?.name
);
const typeName = computed(
  () =>
    typeOptions.value.find((item) => item.id === roomInfo.value.typeId)?.name
);

watch(props, (props) => {
  if (props.editingRoomInfo) {
    roomInfo.value = props.editingRoomInfo;
    setTypeOptions(props.editingRoomInfo.hotelId || 0);
  }
});
...
const deletePriceItem = (priceItemIndex: number) =>
  showConfirmDialog({ title: "确定删除该房间价格吗？" })
    .then(() => roomInfo.value.priceList.splice(priceItemIndex, 1))
    .catch(() => true);
const dateRangeConfirm = (dateList: Date[]) => {
  const priceItem = _.cloneDeep(
    roomInfo.value.priceList[curPriceItemIndex.value]
  );
  roomInfo.value.priceList[curPriceItemIndex.value] = {
    ...priceItem,
...
const save = async () => {
  if (!checkRoomInfo(roomInfo.value)) {
    return;
  }

  const { cancellable, ...rest } = roomInfo.value;
  emit("save", {
    roomInfo: {
      ...rest,
