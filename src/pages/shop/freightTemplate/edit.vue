...
const freightTemplate = ref<ExistFreightTemplate>({
  id: 0,
  mode: 1,
  name: "",
  title: "",
...
const allExpressSelectDisabled = computed(
  () =>
    ...
    expressOptions.value.findIndex(
      (item) =>
        item.expressId ===
        freightTemplate.value.expressList[curExpressIndex.value].id
    ) === -1
);
...
const selectAllExpress = (value: boolean) => {
  if (curExpressOptionIndex.value === -1) {
    const curExpressItem =
      freightTemplate.value.expressList[curExpressIndex.value];
    if (value) {
...
const addExpress = () =>
  freightTemplate.value.expressList.push({
    id: freightTemplate.value.expressList.length
      ? freightTemplate.value.expressList[
          freightTemplate.value.expressList.length - 1
        ].id + 1
      : 1,
    pickedExpressCodes: [],
    pickedExpressDescs: [],
...
const deleteExpress = (index: number) => {
  showConfirmDialog({ title: "确定删除该快递方式配置吗？" })
    .then(() => {
      curExpressIndex.value = index;
      curExpressOptionIndex.value = -2;
      const id = freightTemplate.value.expressList[index].id;
      expressOptions.value = expressOptions.value.map((item) => {
      ...
      freightTemplate.value.expressList.splice(index, 1);
      return true;
    })
    .catch(() => true);
};
...
const expressAllAreaSelectDisabled = computed(
  () =>
    ...
    regionOptionsList.value[curExpressTemplateListIndex.value].list.findIndex(
      (item) =>
        item.areaIds.includes(
          freightTemplate.value.expressTemplateLists[
...
const addExpressTemplateList = (index: number) => {
  const curList = freightTemplate.value.expressTemplateLists[index].list;
  curList.push({
...
const deleteExpressTemplateList = (index: number, _index: number) =>
  showConfirmDialog({ title: "确定删除该快递模版配置吗？" })
    .then(() => {
      ...
      expressPickedCityIndex.value = -3;
      const id =
        freightTemplate.value.expressTemplateLists[index].list[_index].id;
      ...
      freightTemplate.value.expressTemplateLists[index].list.splice(_index, 1);
      return true;
    })
    .catch(() => true);
...
const save = async () => {
  if (!freightTemplate.value.name) {
    showToast("请输入模板名称");
    return;
  }
  if (!freightTemplate.value.title) {
    showToast("请输入模板标题");
    return;
  }
  if (
    freightTemplate.value.mode === 2 &&
    !freightTemplate.value.expressTemplateLists.length
  ) {
    showToast("请添加快递模板");
    return;
  ...
  try {
    await editFreightTemplate({
      ...
      areaList: JSON.stringify(freightTemplate.value.areaList),
      expressList: JSON.stringify(freightTemplate.value.expressList),
